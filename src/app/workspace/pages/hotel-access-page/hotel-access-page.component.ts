...
export class HotelAccessPageComponent {
  ...
  async onAccessSubmit() {
    ...
    this.hotelSessionService
      .validateAccess(hotelId, password)
      .subscribe((isAccessOk) => {
        if (isAccessOk) {
          this.router.navigateByUrl('/hotelsession'); // Si la autenticación es exitosa, redirige al usuario a la página principal.
