...
export class IncidentsService {
  ...
  getIncidentsByHotelId(hotelId: number | undefined): Observable<Incident[]> {
    return this.http
      .get<Incident[]>(`${baseUrl}/hotels/${hotelId}/incidents`)
      .pipe(
  ...
  deleteIncident(incidentId: number): Observable<void> {
    ...
    return this.http
      ...
      .pipe(
        tap(() => {
          this.incidents.set(
            this.incidents().filter((i) => i.id !== incidentId)
          );
        })
      );
  }
  ...
  getIncidentById(id: number): Observable<Incident> {
    ...
    if (cachedIncident)
      return new Observable((obs) => {
        obs.next(cachedIncident);
        obs.complete();
      });
    return this.http
      .get<Incident>(`${baseUrl}/hotels/${hotelId}/incidents/${id}`)
      .pipe(
        tap((incident) => {
          const arr = this.incidents();
          if (!arr.find((i) => i.id === incident.id))
            this.incidents.set([...arr, incident]);
        }),
        catchError((error) => {
          throw error;
        })
  ...
  updateIncident(
    ...
    return this.http
      .put<Incident>(
        `${baseUrl}/hotels/${hotelId}/incidents/${incidentId}`,
        data
      )
      .pipe(
        tap((incident) => {
          this.incidents.set(
            this.incidents().map((i) => (i.id === incidentId ? incident : i))
          );
        })
      );
  }
  ...
  createIncident(data: Omit<Incident, 'id'>): Observable<Incident> {
    const hotelId = this.hotelSessionService.hotelSession()?.hotelId;
    return this.http
      .post<Incident>(`${baseUrl}/hotels/${hotelId}/incidents`, data)
      .pipe(
        tap((incident) => {
